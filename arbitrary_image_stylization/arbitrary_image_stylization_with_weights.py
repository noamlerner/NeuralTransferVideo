...
flags.DEFINE_string('image_size', '[256]', 'Image size.')
flags.DEFINE_boolean('content_square_crop', False, 'Whether to center crop'
                                                   'the content image to be a square or not.')
flags.DEFINE_string('style_image_size', '[256]', 'List of Style image size.')
...
def main(unused_argv=None):
    for image_size in ast.literal_eval(FLAGS.image_size):
        for style_image_size in ast.literal_eval(FLAGS.style_image_size):
            iter_f(style_image_size, image_size)


def iter_f(style_image_size, image_size):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        content_img_list = FLAGS.content_images_paths.split(",")
            ...
            for style_i, style_img_path in enumerate(style_img_list):
                ...
                for interp_i, wi in enumerate(interpolation_weights):
                    ...
                    image_utils.save_np_image(
                        stylized_image_res,
                        os.path.join(FLAGS.output_dir, '%s_stylized_%s_%s_%s_%s.jpg' %
                                     (content_img_name, style_img_name, style_image_size, image_size, interpolation_weights[0])))
